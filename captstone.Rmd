...
```{r}
ggplot(hcaphs_df_measures,aes(HCAHPS.Measure.ID,HCAHPS.Linear.Mean.Value)) + geom_col() + theme(axis.text.x = element_text(angle = 45,hjust = 1, vjust = 1)) 
```
...
```{r}
levels(hcaphs_df_measures_distri$HCAHPS.Measure.ID)
...
```{r}
hcaphs_df_final <- hcaphs_df_measures_distri %>% spread(key = HCAHPS.Measure.ID, value = HCAHPS.Linear.Mean.Value)
summary(hcaphs_df_final)
remove(hcaphs_df_measures,hcaphs_df_measures_distri)
```
