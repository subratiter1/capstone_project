...
```{r}
ggplot(complications_df_cleaned_footnote_nas,aes(Measure.ID)) + geom_bar() + theme(axis.text.x = element_text(angle = 90))
```
...
complications_df_cleaned_footnote_nas %>% group_by(Measure.ID) %>% 
  summarise(cnt_rws = n(), Sum_score = sum(Score,na.rm = T),
            Sum_Denom = sum(Denominator,na.rm = T)) %>% 
  arrange(desc(cnt_rws)) %>% melt(value.name = c("value")) %>%
  ggplot(aes(x = Measure.ID,y = value)) + geom_col() + facet_wrap(facets = ~ variable,scales = "free") + theme(axis.text.x = element_text(angle = 90))
...
- Significant Measures from these measure Ids are "PSI_90_SAFETY","PSI_4_SURG_COMP"
- "COMP_HIP_KNEE" is the other significant measure
...
```{r}
zvar1 <- which(names(complications_df_spread) %in% c("Provider.ID","PSI_4_SURG_COMP","PSI_90_SAFETY","COMP_HIP_KNEE"))
...
hai_df_cleaned <- hai_df[,-which(names(hai_df_footnote) %in% c(zdemographics))]
str(hai_df_cleaned)
...
```{r}
hai_df %>% group_by(Footnote) %>% summarise(count_rows = n(),score_total = sum(Score,na.rm = T))
```
...
* There are abnormal outliers in the dataset from the measure IDs HAI_1_DOPC_Days and as they do not contribute to the overall score
...
HAI-2a measure only tracks catheter-associated urinary tract infections (CAUTI) in ICUs. 
...
- selecting the required columns for analyysis
