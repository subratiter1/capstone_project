...
* But the dataset contains the Acute care, Childrens, and Critical Access Hosptials. Hence filtering out all the acute care hosptials from the General Information dataset
...
#general_info_cleaned <-  general_info_cleaned[general_info_cleaned$Hospital.Type %in% "Acute Care Hospitals",- zvar2]
general_info_cleaned <-  general_info_cleaned[,-zvar2]
...
- Patient experience measures are uniformly distributed about their mean
...
```{r}
time_and_eff_care_cleaned %>% group_by(Footnote) %>% tally(sort = T)
```
