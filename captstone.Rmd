...
```{r}
library(psych)
pairs.panels(x = complications_df_spread[2:ncol(complications_df_spread)],
             bg = rainbow(n = 12),
             smooth = TRUE,
...
- Some of the variabels are very variability for example, "PSI_90_SAFETY","COMP_HIP_KNEE" exhibit high variability compared to other measures. 
...
- Extracting the Measure IDs of importance are:
...
* selecting the above measures from the 
...
```
```{r}
n_vars <- ncol(patient_exp_group)

pairs.panels(patient_exp_group[,2:n_vars],cex.labels = 0.7 ,cex.main = 0.7,main = "Relationship between measures")#,colors = rainbow(n_vars))
...
* considering the Footnote leves: 1. Blank Levels 2.  "2 - Data submitted were based on a sample of cases/patients."
* 

```{r}
time_and_eff_care_cleaned$Score <- as.integer(time_and_eff_care_cleaned$Score)
time_and_eff_care_cleaned[!is.na(time_and_eff_care_cleaned$Score),] %>% summary()
time_and_eff_care_cleaned[is.na(time_and_eff_care_cleaned$Score),] %>% View()

time_and_eff_care_cleaned[,] %>%summary()
time_and_eff_care_cleaned[is.na(time_and_eff_care_cleaned$Footnote),] %>% View
...
* Measures that do not contribute less to the overall score and that are have lesser providers ids are eliminated from the analysis
...
```{r}
pairs.panels(x = timeliness_group[2:length(zvar1)],cex.cor = 0.5,cex.labels = 0.9,ellipses = TRUE,pch = 21,bg = rainbow(length(zvar1)))
```
...
```
- Variables represented about have small degree of correlation
...
```{r}
zdemogrphic_vars <- which(names(readm_mort_df) %in% zdemographics)
...
* Considering the Footnote variable as the reference variable. Assuming that the
...
```{r}
readm_mort_cleaned[is.na(readm_mort_cleaned$Footnote),] %>% summary()
...
* All varaibles in the dataset are significant and thus are being retained into the final 
* Two variables that are significant are significant correlated 
...
```
*  from the summary we can infer that most of the NAs in the score are induced by Stale data identified from different classess of Footnotes
