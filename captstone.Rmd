...
```{r}
set.seed(100)
ratings <- (master_df$Hospital.overall.rating)

as.character(ratings[which(is.na(ratings))]) <- "Missing"
summary(ratings)

indices <- sample(1:nrow(master_df),0.7 * nrow(master_df))

# training dataset
train_df <- master_df[indices,]
# test dataset
test_df <- master_df[-indices,]
```
